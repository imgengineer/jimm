...
class InceptionE(nnx.Module):  # 8x8 grid
    def __init__(self, in_chs, *, rngs):
        ...
        self.b2_1 = ConvBNAct(in_chs, 384, 1, rngs=rngs)
        self.b2_2a = ConvBNAct(384, 384, (1, 3), padding="SAME", rngs=rngs)
        self.b2_2b = ConvBNAct(384, 384, (3, 1), padding="SAME", rngs=rngs)
        self.b3_1 = ConvBNAct(in_chs, 448, 1, rngs=rngs)
        self.b3_2 = ConvBNAct(448, 384, 3, padding="SAME", rngs=rngs)
        self.b3_3a = ConvBNAct(384, 384, (1, 3), padding="SAME", rngs=rngs)
        self.b3_3b = ConvBNAct(384, 384, (3, 1), padding="SAME", rngs=rngs)
        self.b4 = ConvBNAct(in_chs, 192, 1, rngs=rngs)

    def __call__(self, x):
        b1 = self.b1(x)
        b2 = self.b2_1(x)
        b2 = jnp.concatenate([self.b2_2a(b2), self.b2_2b(b2)], axis=-1)
        b3 = self.b3_2(self.b3_1(x))
        b3 = jnp.concatenate([self.b3_3a(b3), self.b3_3b(b3)], axis=-1)
        b4 = self.b4(nnx.avg_pool(x, (3, 3), strides=(1, 1), padding="SAME"))
