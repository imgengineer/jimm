...
class ReductionA(nnx.Module):  # 35 -> 17 (or 71 -> 35)
    def __init__(self, in_chs, k, mid_chs, m, n, *, rngs):
        self.b1 = ConvBNAct(in_chs, n, 3, 2, padding="VALID", rngs=rngs)
        self.b2 = nnx.List(
            [
                ConvBNAct(in_chs, k, 1, rngs=rngs),
                ConvBNAct(k, mid_chs, 3, rngs=rngs),
                ConvBNAct(mid_chs, m, 3, 2, padding="VALID", rngs=rngs),
            ]
        )
    ...
    def __init__(self, in_chs, *, rngs):
        ...
        self.b2 = nnx.List(
            [
                ...
                ConvBNAct(192, 224, (1, 7), rngs=rngs),
        ...
        self.b3 = nnx.List(
            [
                ...
                ConvBNAct(192, 224, (1, 7), rngs=rngs),
                ConvBNAct(224, 224, (7, 1), rngs=rngs),
...
class ReductionB(nnx.Module):  # 17 -> 8
    def __init__(self, in_chs, *, rngs):
        ...
        self.b2 = nnx.List(
            [
                ConvBNAct(in_chs, 256, 1, rngs=rngs),
                ConvBNAct(256, 256, (1, 7), rngs=rngs),
...
class InceptionC(nnx.Module):
    def __init__(self, in_chs, *, rngs):
        ...
        self.b3a = nnx.List(
            [
                ConvBNAct(in_chs, 384, 1, rngs=rngs),
                ConvBNAct(384, 448, (3, 1), rngs=rngs),
...
class InceptionV4(ClassifierMixin, nnx.Module):
    def __init__(self, num_classes=1000, in_chans=3, global_pool="avg", drop_rate=0.0, *, rngs):
        ...
        self.s4a_b1 = nnx.List(
            [ConvBNAct(160, 64, 1, rngs=rngs), ConvBNAct(64, 96, 3, padding="VALID", rngs=rngs)]
        )
        self.s4a_b2 = nnx.List(
            [
                ConvBNAct(160, 64, 1, rngs=rngs),
                ConvBNAct(64, 64, (7, 1), rngs=rngs),
    ...
    def forward_features(self, x):
        ...
        x = jnp.concatenate(
            [
                self.s4a_b1[1](self.s4a_b1[0](x)),
                self.s4a_b2[3](self.s4a_b2[2](self.s4a_b2[1](self.s4a_b2[0](x)))),
            ],
            axis=-1,
        )
