...
class OSABlock(nnx.Module):
    ...
    def __init__(self, in_chs, mid_chs, out_chs, layers=5, *, rngs):
        ...
        convs = [
            ConvBNAct(in_chs if i == 0 else mid_chs, mid_chs, 3, rngs=rngs) for i in range(layers)
        ]
...
_CFGS = {
    ...
    "ese_vovnet39b": (
        [(128, 256, 1, 5), (160, 512, 2, 5), (192, 768, 3, 5), (224, 1024, 2, 5)],
        True,
    ),
}
    ...
    def __init__(
        ...
        *,
        ...
        self.stages = nnx.List(
            [
                VoVNetStage(
                    128 if i == 0 else stages_cfg[i - 1][1], mid, out, blocks, lpb, ese, rngs=rngs
                )
