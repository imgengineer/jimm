        ...
        *,
        ...
        self.conv_stages = nnx.List(
            [
                nnx.List(
                    [
                        MbConvLNBlock(channels[0], channels[0], 1, rngs=rngs)
                        for _ in range(conv_depths[0])
                    ]
                ),
                nnx.List(
                    [
                        MbConvLNBlock(
                            channels[0] if j == 0 else channels[1],
                            channels[1],
                            2 if j == 0 else 1,
                            rngs=rngs,
                        )
        ...
        self.vit_stage2 = nnx.List(
            [VitBlock(channels[2], num_heads[0], rngs=rngs) for _ in range(vit_depths[0])]
        )
        self.down3 = ConvBNAct(channels[2], channels[3], 3, 2, act="identity", rngs=rngs)
        self.vit_stage3 = nnx.List(
            [VitBlock(channels[3], num_heads[1], rngs=rngs) for _ in range(vit_depths[1])]
        )
