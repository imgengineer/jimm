...
class TinyViT(ClassifierMixin, nnx.Module):
    def __init__(
        ...
        *,
        ...
        self.conv_stage = nnx.List(
            [InvertedResidual(channels[0], channels[0], 1, 4, rngs=rngs) for _ in range(depths[0])]
        )
        attn_stages, k = [], depths[0]
        for i in range(1, 4):
            c, d, h = channels[i], depths[i], num_heads[i]
            blocks = [
                TinyViTWindowBlock(c, h, window_size, dpr[k + j], rngs=rngs) for j in range(d)
            ]
            k += d
