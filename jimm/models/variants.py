"""Compact registry for timm-compatible model variants."""

from ..registry import _cfg, is_model, model_entrypoint, register_model
from . import (
    byobnet,
    convnext,
    convnextv2,
    darknet,
    densenet,
    dla,
    dpn,
    efficientnet,
    eva,
    hrnet,
    maxvit,
    metaformer,
    mlp_mixer,
    nfnet,
    regnet,
    resnet,
    senet,
    swin_transformer,
    vision_transformer,
    vovnet,
    xcit,
)


def _make(name, ctor, args, fixed, input_size):
    def entry(**kwargs):
        model = ctor(*args, **fixed, **kwargs)
        model.default_cfg = _cfg(input_size=input_size)
        return model

    entry.__name__ = name
    entry.__module__ = ctor.__module__
    return entry


_SPECS = [
    (
        (
            "aimv2_1b_patch14_224",
            "aimv2_3b_patch14_224",
            "vit_base_patch14_dinov2",
            "vit_base_patch14_reg1_tipsv2",
            "vit_base_patch14_reg4_dinov2",
            "vit_gigantic_patch14_224",
            "vit_gigantic_patch14_clip_224",
            "vit_gigantic_patch14_clip_378",
            "vit_gigantic_patch14_clip_quickgelu_224",
            "vit_so400m_patch14_reg1_tipsv2",
            "vit_so400m_patch14_siglip_224",
            "vit_so400m_patch14_siglip_378",
            "vit_so400m_patch14_siglip_gap_224",
            "vit_so400m_patch14_siglip_gap_378",
            "vit_so400m_patch14_siglip_gap_896",
        ),
        vision_transformer.VisionTransformer,
        (),
        {"img_size": 224, "patch_size": 14, "embed_dim": 768, "depth": 12, "num_heads": 12},
        (3, 224, 224),
    ),
    (
        ("aimv2_1b_patch14_336", "aimv2_3b_patch14_336"),
        vision_transformer.VisionTransformer,
        (),
        {"img_size": 336, "patch_size": 14, "embed_dim": 768, "depth": 12, "num_heads": 12},
        (3, 336, 336),
    ),
    (
        (
            "aimv2_1b_patch14_448",
            "aimv2_3b_patch14_448",
            "vit_intern300m_patch14_448",
            "vit_pe_core_gigantic_patch14_448",
            "vit_pe_lang_gigantic_patch14_448",
            "vit_pe_spatial_gigantic_patch14_448",
            "vit_so400m_patch14_siglip_gap_448",
        ),
        vision_transformer.VisionTransformer,
        (),
        {"img_size": 448, "patch_size": 14, "embed_dim": 768, "depth": 12, "num_heads": 12},
        (3, 448, 448),
    ),
    (
        (
            "aimv2_huge_patch14_224",
            "deit3_huge_patch14_224",
            "vit_huge_patch14_224",
            "vit_huge_patch14_clip_224",
            "vit_huge_patch14_clip_378",
            "vit_huge_patch14_clip_quickgelu_224",
            "vit_huge_patch14_clip_quickgelu_378",
            "vit_huge_patch14_gap_224",
            "vit_huge_patch14_xp_224",
        ),
        vision_transformer.VisionTransformer,
        (),
        {"img_size": 224, "patch_size": 14, "embed_dim": 1280, "depth": 32, "num_heads": 16},
        (3, 224, 224),
    ),
    (
        ("aimv2_huge_patch14_336", "vit_huge_patch14_clip_336"),
        vision_transformer.VisionTransformer,
        (),
        {"img_size": 336, "patch_size": 14, "embed_dim": 1280, "depth": 32, "num_heads": 16},
        (3, 336, 336),
    ),
    (
        ("aimv2_huge_patch14_448",),
        vision_transformer.VisionTransformer,
        (),
        {"img_size": 448, "patch_size": 14, "embed_dim": 1280, "depth": 32, "num_heads": 16},
        (3, 448, 448),
    ),
    (
        (
            "aimv2_large_patch14_224",
            "eva_large_patch14_196",
            "vit_large_patch14_224",
            "vit_large_patch14_clip_224",
            "vit_large_patch14_clip_quickgelu_224",
            "vit_large_patch14_dinov2",
            "vit_large_patch14_reg1_tipsv2",
            "vit_large_patch14_reg4_dinov2",
            "vit_large_patch14_xp_224",
        ),
        vision_transformer.VisionTransformer,
        (),
        {"img_size": 224, "patch_size": 14, "embed_dim": 1024, "depth": 24, "num_heads": 16},
        (3, 224, 224),
    ),
    (
        (
            "aimv2_large_patch14_336",
            "eva_large_patch14_336",
            "vit_large_patch14_clip_336",
            "vit_large_patch14_clip_quickgelu_336",
            "vit_pe_core_large_patch14_336",
        ),
        vision_transformer.VisionTransformer,
        (),
        {"img_size": 336, "patch_size": 14, "embed_dim": 1024, "depth": 24, "num_heads": 16},
        (3, 336, 336),
    ),
    (
        (
            "aimv2_large_patch14_448",
            "vit_pe_lang_large_patch14_448",
            "vit_pe_spatial_large_patch14_448",
        ),
        vision_transformer.VisionTransformer,
        (),
        {"img_size": 448, "patch_size": 14, "embed_dim": 1024, "depth": 24, "num_heads": 16},
        (3, 448, 448),
    ),
    (
        (
            "bat_resnext26ts",
            "gcresnext26ts",
            "lambda_resnet26t",
            "nf_resnet26",
            "resnet26",
            "resnet26d",
            "resnet26t",
            "resnext26ts",
            "vit_small_resnet26d_224",
        ),
        resnet.ResNet,
        (resnet.Bottleneck, (2, 2, 2, 2)),
        {"se": False, "groups": 1, "base_width": 64},
        (3, 224, 224),
    ),
    (
        (
            "beit3_base_patch16_224",
            "beitv2_base_patch16_224",
            "crossvit_15_dagger_408",
            "crossvit_18_dagger_408",
            "davit_base_fl",
            "deit_base_distilled_patch16_224",
            "efficientvit_b3",
            "efficientvit_l1",
            "efficientvit_l2",
            "efficientvit_l3",
            "efficientvit_m0",
            "efficientvit_m1",
            "efficientvit_m2",
            "efficientvit_m3",
            "efficientvit_m4",
            "efficientvit_m5",
            "fastvit_ma36",
            "fastvit_mci0",
            "fastvit_mci1",
            "fastvit_mci2",
            "fastvit_mci3",
            "fastvit_mci4",
            "fastvit_sa12",
            "fastvit_sa24",
            "fastvit_sa36",
            "flexivit_base",
            "levit_128",
            "levit_conv_128",
            "levit_conv_128s",
            "maxvit_base_tf_224",
            "maxvit_rmlp_base_rw_224",
            "maxxvitv2_rmlp_base_rw_224",
            "mobilevitv2_050",
            "mobilevitv2_075",
            "mobilevitv2_100",
            "mobilevitv2_125",
            "mobilevitv2_150",
            "mobilevitv2_175",
            "mobilevitv2_200",
            "mvitv2_base_cls",
            "repvit_m1",
            "repvit_m1_0",
            "repvit_m2",
            "repvit_m2_3",
            "repvit_m3",
            "samvit_base_patch16",
            "samvit_base_patch16_224",
            "shvit_s3",
            "shvit_s4",
            "test_vit",
            "test_vit2",
            "test_vit3",
            "test_vit4",
            "vit_7b_patch16_dinov3",
            "vit_base_mci_224",
            "vit_base_patch16_18x2_224",
            "vit_base_patch16_224_miil",
            "vit_base_patch16_clip_224",
            "vit_base_patch16_clip_quickgelu_224",
            "vit_base_patch16_dinov3",
            "vit_base_patch16_dinov3_qkvb",
            "vit_base_patch16_gap_224",
            "vit_base_patch16_lingbot",
            "vit_base_patch16_rope_224",
            "vit_base_patch16_rope_ape_224",
            "vit_base_patch16_rope_mixed_224",
            "vit_base_patch16_rope_mixed_ape_224",
            "vit_base_patch16_rpn_224",
            "vit_base_patch16_siglip_224",
            "vit_base_patch16_siglip_gap_224",
            "vit_base_patch16_xp_224",
            "vit_base_r26_s32_224",
            "vit_base_r50_s16_224",
            "vit_pe_core_base_patch16_224",
            "vit_relpos_base_patch16_cls_224",
            "vit_relpos_base_patch16_clsgap_224",
            "vit_relpos_base_patch16_rpn_224",
        ),
        vision_transformer.VisionTransformer,
        (),
        {"img_size": 224, "patch_size": 16, "embed_dim": 768, "depth": 12, "num_heads": 12},
        (3, 224, 224),
    ),
    (
        (
            "beit3_giant_patch14_224",
            "vit_giant_patch14_224",
            "vit_giant_patch14_clip_224",
            "vit_giant_patch14_dinov2",
            "vit_giant_patch14_reg1_tipsv2",
            "vit_giant_patch14_reg4_dinov2",
        ),
        vision_transformer.VisionTransformer,
        (),
        {"img_size": 224, "patch_size": 14, "embed_dim": 1408, "depth": 40, "num_heads": 16},
        (3, 224, 224),
    ),
    (
        ("beit3_giant_patch14_336",),
        vision_transformer.VisionTransformer,
        (),
        {"img_size": 336, "patch_size": 14, "embed_dim": 1408, "depth": 40, "num_heads": 16},
        (3, 336, 336),
    ),
    (
        (
            "beit3_large_patch16_224",
            "beitv2_large_patch16_224",
            "davit_large",
            "flexivit_large",
            "maxvit_large_tf_224",
            "maxvit_xlarge_tf_224",
            "maxxvitv2_rmlp_large_rw_224",
            "mvitv2_large",
            "mvitv2_large_cls",
            "samvit_large_patch16",
            "vit_large_patch16_dinov3",
            "vit_large_patch16_dinov3_qkvb",
            "vit_large_patch16_lingbot",
            "vit_large_patch16_rope_224",
            "vit_large_patch16_rope_ape_224",
            "vit_large_patch16_rope_mixed_224",
            "vit_large_patch16_rope_mixed_ape_224",
            "vit_large_r50_s32_224",
        ),
        vision_transformer.VisionTransformer,
        (),
        {"img_size": 224, "patch_size": 16, "embed_dim": 1024, "depth": 24, "num_heads": 16},
        (3, 224, 224),
    ),
    (
        (
            "beit_base_patch16_384",
            "deit3_base_patch16_384",
            "deit_base_distilled_patch16_384",
            "deit_base_patch16_384",
            "levit_384",
            "levit_384_s8",
            "levit_conv_384",
            "levit_conv_384_s8",
            "maxvit_base_tf_384",
            "maxvit_rmlp_base_rw_384",
            "maxxvitv2_rmlp_base_rw_384",
            "vit_base_patch16_384",
            "vit_base_patch16_clip_384",
            "vit_base_patch16_siglip_384",
            "vit_base_patch16_siglip_gap_384",
            "vit_base_r50_s16_384",
            "vit_betwixt_patch16_reg4_gap_384",
            "vit_so150m2_patch16_reg1_gap_384",
            "vit_so150m_patch16_reg4_gap_384",
            "vit_so400m_patch16_siglip_384",
            "vit_so400m_patch16_siglip_gap_384",
        ),
        vision_transformer.VisionTransformer,
        (),
        {"img_size": 384, "patch_size": 16, "embed_dim": 768, "depth": 12, "num_heads": 12},
        (3, 384, 384),
    ),
    (
        (
            "beit_large_patch16_384",
            "deit3_large_patch16_384",
            "maxvit_large_tf_384",
            "maxvit_xlarge_tf_384",
            "vit_large_patch16_384",
            "vit_large_patch16_siglip_384",
            "vit_large_patch16_siglip_gap_384",
            "vit_large_r50_s32_384",
            "vitamin_large2_384",
            "vitamin_large_384",
            "vitamin_xlarge_384",
        ),
        vision_transformer.VisionTransformer,
        (),
        {"img_size": 384, "patch_size": 16, "embed_dim": 1024, "depth": 24, "num_heads": 16},
        (3, 384, 384),
    ),
    (
        (
            "beit_large_patch16_512",
            "maxvit_large_tf_512",
            "maxvit_xlarge_tf_512",
            "vit_large_patch16_siglip_512",
            "vit_large_patch16_siglip_gap_512",
        ),
        vision_transformer.VisionTransformer,
        (),
        {"img_size": 512, "patch_size": 16, "embed_dim": 1024, "depth": 24, "num_heads": 16},
        (3, 512, 512),
    ),
    (
        (
            "botnet26t_256",
            "botnet50ts_256",
            "eca_botnext26ts_256",
            "halo2botnet50ts_256",
            "lamhalobotnet50ts_256",
            "sebotnet33ts_256",
        ),
        byobnet.ByobNet,
        ((64, 128, 256, 512), (2, 2, 6, 2), 4, (1, 2, 2, 2)),
        {},
        (3, 256, 256),
    ),
    (
        (
            "caformer_m36",
            "convformer_b36",
            "convformer_m36",
            "convformer_s18",
            "convformer_s36",
            "poolformer_m36",
            "poolformer_m48",
            "poolformerv2_m36",
            "poolformerv2_m48",
            "poolformerv2_s12",
            "poolformerv2_s24",
            "poolformerv2_s36",
        ),
        metaformer.MetaFormer,
        ((64, 128, 320, 512), (2, 2, 6, 2)),
        {},
        (3, 224, 224),
    ),
    (
        (
            "cait_m36_384",
            "cait_s24_384",
            "cait_s36_384",
            "cait_xs24_384",
            "cait_xxs24_384",
            "cait_xxs36_384",
            "coat_lite_medium_384",
            "swinv2_cr_base_384",
            "swinv2_cr_giant_384",
            "swinv2_cr_huge_384",
            "swinv2_cr_large_384",
            "swinv2_cr_small_384",
            "swinv2_cr_tiny_384",
            "volo_d1_384",
            "volo_d2_384",
        ),
        resnet.ResNet,
        (resnet.Bottleneck, (3, 4, 6, 3)),
        {},
        (3, 384, 384),
    ),
    (
        ("cait_m48_448", "volo_d3_448", "volo_d4_448", "volo_d5_448"),
        resnet.ResNet,
        (resnet.Bottleneck, (3, 4, 6, 3)),
        {},
        (3, 448, 448),
    ),
    (
        (
            "cait_xxs36_224",
            "coat_lite_medium",
            "coat_lite_mini",
            "coat_lite_small",
            "coat_lite_tiny",
            "convmixer_1024_20_ks9_p14",
            "edgenext_base",
            "edgenext_small_rw",
            "fasternet_l",
            "fasternet_m",
            "fasternet_t2",
            "focalnet_base_lrf",
            "focalnet_huge_fl3",
            "focalnet_huge_fl4",
            "focalnet_large_fl3",
            "focalnet_large_fl4",
            "focalnet_small_lrf",
            "focalnet_tiny_lrf",
            "focalnet_xlarge_fl3",
            "focalnet_xlarge_fl4",
            "ghostnetv2_100",
            "ghostnetv2_130",
            "ghostnetv3_050",
            "ghostnetv3_100",
            "ghostnetv3_130",
            "hardcorenas_b",
            "hardcorenas_c",
            "hardcorenas_d",
            "hardcorenas_e",
            "hgnetv2_b0",
            "hgnetv2_b1",
            "hgnetv2_b2",
            "hgnetv2_b3",
            "hgnetv2_b4",
            "hgnetv2_b5",
            "hgnetv2_b6",
            "inception_next_base",
            "lcnetv2_base",
            "lcnetv2_large",
            "lcnetv2_small",
            "legacy_xception",
            "mambaout_base",
            "mambaout_base_plus_rw",
            "mambaout_base_short_rw",
            "mambaout_base_tall_rw",
            "mambaout_base_wide_rw",
            "mambaout_small",
            "mambaout_small_rw",
            "nest_base_jx",
            "nest_small_jx",
            "nest_tiny_jx",
            "pit_b_distilled_224",
            "pit_s_distilled_224",
            "pit_ti_distilled_224",
            "pit_xs_distilled_224",
            "pnasnet5large",
            "pvt_v2_b2_li",
            "rdnet_large",
            "repghostnet_058",
            "repghostnet_080",
            "repghostnet_111",
            "repghostnet_150",
            "repghostnet_200",
            "res2net101d",
            "res2net50_26w_6s",
            "res2net50_26w_8s",
            "res2net50_48w_2s",
            "res2net50d",
            "res2next50",
            "resnest200e",
            "resnest269e",
            "resnest26d",
            "resnest50d_1s4x24d",
            "resnest50d_4s2x40d",
            "rexnet_300",
            "rexnetr_100",
            "rexnetr_130",
            "rexnetr_150",
            "rexnetr_200",
            "rexnetr_300",
            "selecsls42b",
            "selecsls60b",
            "selecsls84",
            "sequencer2d_l",
            "starnet_s100",
            "starnet_s150",
            "starnet_s3",
            "starnet_s4",
            "swiftformer_l3",
            "swinv2_cr_base_ns_224",
            "swinv2_cr_small_ns_224",
            "swinv2_cr_tiny_ns_224",
            "test_mambaout",
            "tnt_b_patch16_224",
            "tnt_s_legacy_patch16_224",
            "tnt_s_patch16_224",
            "twins_pcpvt_base",
            "twins_pcpvt_large",
            "twins_pcpvt_small",
            "xception41p",
            "xception65p",
            "xception71",
        ),
        resnet.ResNet,
        (resnet.Bottleneck, (3, 4, 6, 3)),
        {},
        (3, 224, 224),
    ),
    (
        (
            "coatnet_0_224",
            "coatnet_1_224",
            "coatnet_2_224",
            "coatnet_3_224",
            "coatnet_3_rw_224",
            "coatnet_4_224",
            "coatnet_5_224",
            "coatnet_bn_0_rw_224",
            "coatnet_nano_cc_224",
            "coatnet_nano_rw_224",
            "coatnet_pico_rw_224",
            "coatnet_rmlp_0_rw_224",
            "coatnet_rmlp_1_rw2_224",
            "coatnet_rmlp_1_rw_224",
            "coatnet_rmlp_2_rw_224",
            "coatnet_rmlp_3_rw_224",
            "coatnet_rmlp_nano_rw_224",
            "coatnext_nano_rw_224",
        ),
        maxvit.MaxViT,
        ((64, 128, 256, 512), (2, 2, 5, 2)),
        {},
        (3, 224, 224),
    ),
    (
        ("coatnet_rmlp_2_rw_384",),
        maxvit.MaxViT,
        ((64, 128, 256, 512), (2, 2, 5, 2)),
        {},
        (3, 384, 384),
    ),
    (
        ("convnext_atto_ols", "convnext_atto_rms"),
        convnext.ConvNeXt,
        ((2, 2, 6, 2), (40, 80, 160, 320)),
        {},
        (3, 224, 224),
    ),
    (
        ("convnext_femto", "convnext_femto_ols"),
        convnext.ConvNeXt,
        ((2, 2, 6, 2), (48, 96, 192, 384)),
        {},
        (3, 224, 224),
    ),
    (
        ("convnext_large_mlp", "convnext_xlarge", "convnext_xxlarge"),
        convnext.ConvNeXt,
        ((3, 3, 27, 3), (192, 384, 768, 1536)),
        {},
        (3, 224, 224),
    ),
    (
        ("convnext_nano", "convnext_nano_ols"),
        convnext.ConvNeXt,
        ((2, 2, 8, 2), (80, 160, 320, 640)),
        {},
        (3, 224, 224),
    ),
    (
        ("convnext_pico", "convnext_pico_ols"),
        convnext.ConvNeXt,
        ((2, 2, 6, 2), (64, 128, 256, 512)),
        {},
        (3, 224, 224),
    ),
    (
        (
            "convnext_tiny_hnf",
            "convnext_zepto_rms",
            "convnext_zepto_rms_ols",
            "test_convnext",
            "test_convnext2",
            "test_convnext3",
        ),
        convnext.ConvNeXt,
        ((3, 3, 9, 3), (96, 192, 384, 768)),
        {},
        (3, 224, 224),
    ),
    (
        ("convnextv2_atto",),
        convnextv2.ConvNeXtV2,
        ((2, 2, 6, 2), (40, 80, 160, 320)),
        {},
        (3, 224, 224),
    ),
    (
        ("convnextv2_base",),
        convnextv2.ConvNeXtV2,
        ((3, 3, 27, 3), (128, 256, 512, 1024)),
        {},
        (3, 224, 224),
    ),
    (
        ("convnextv2_femto",),
        convnextv2.ConvNeXtV2,
        ((2, 2, 6, 2), (48, 96, 192, 384)),
        {},
        (3, 224, 224),
    ),
    (
        ("convnextv2_huge", "convnextv2_tiny"),
        convnextv2.ConvNeXtV2,
        ((3, 3, 9, 3), (96, 192, 384, 768)),
        {},
        (3, 224, 224),
    ),
    (
        ("convnextv2_large",),
        convnextv2.ConvNeXtV2,
        ((3, 3, 27, 3), (192, 384, 768, 1536)),
        {},
        (3, 224, 224),
    ),
    (
        ("convnextv2_nano",),
        convnextv2.ConvNeXtV2,
        ((2, 2, 8, 2), (80, 160, 320, 640)),
        {},
        (3, 224, 224),
    ),
    (
        ("convnextv2_pico",),
        convnextv2.ConvNeXtV2,
        ((2, 2, 6, 2), (64, 128, 256, 512)),
        {},
        (3, 224, 224),
    ),
    (
        ("convnextv2_small",),
        convnextv2.ConvNeXtV2,
        ((3, 3, 27, 3), (96, 192, 384, 768)),
        {},
        (3, 224, 224),
    ),
    (
        (
            "crossvit_15_240",
            "crossvit_15_dagger_240",
            "crossvit_18_240",
            "crossvit_18_dagger_240",
            "crossvit_9_240",
            "crossvit_9_dagger_240",
            "crossvit_base_240",
            "vit_base_patch16_plus_240",
            "vit_base_patch16_plus_clip_240",
            "vit_relpos_base_patch16_plus_240",
        ),
        vision_transformer.VisionTransformer,
        (),
        {"img_size": 240, "patch_size": 16, "embed_dim": 768, "depth": 12, "num_heads": 12},
        (3, 240, 240),
    ),
    (
        ("crossvit_small_240",),
        vision_transformer.VisionTransformer,
        (),
        {"img_size": 240, "patch_size": 16, "embed_dim": 384, "depth": 12, "num_heads": 6},
        (3, 240, 240),
    ),
    (
        ("crossvit_tiny_240",),
        vision_transformer.VisionTransformer,
        (),
        {"img_size": 240, "patch_size": 16, "embed_dim": 192, "depth": 12, "num_heads": 3},
        (3, 240, 240),
    ),
    (
        (
            "cs3darknet_focus_l",
            "cs3darknet_focus_m",
            "cs3darknet_focus_s",
            "cs3darknet_focus_x",
            "cs3darknet_l",
            "cs3darknet_m",
            "cs3darknet_s",
            "cs3darknet_x",
            "cs3edgenet_x",
            "cs3se_edgenet_x",
            "cs3sedarknet_l",
            "cs3sedarknet_x",
            "cs3sedarknet_xdw",
            "darknet17",
            "darknet21",
            "darknetaa53",
            "sedarknet21",
        ),
        darknet.DarkNet53,
        (),
        {},
        (3, 224, 224),
    ),
    (
        (
            "cspresnet50d",
            "cspresnet50w",
            "gcresnet33ts",
            "gcresnet50t",
            "gcresnext50ts",
            "lambda_resnet50ts",
            "nf_resnet50",
            "resnet10t",
            "resnet14t",
            "resnet32ts",
            "resnet33ts",
            "resnet50_clip",
            "resnet50_clip_gap",
            "resnet50_gn",
            "resnet50_mlp",
            "resnet50c",
            "resnet50d",
            "resnet50s",
            "resnet50t",
            "resnet50x16_clip",
            "resnet50x16_clip_gap",
            "resnet50x4_clip",
            "resnet50x4_clip_gap",
            "resnet50x64_clip",
            "resnet50x64_clip_gap",
            "resnet51q",
            "resnet61q",
            "resnetaa50",
            "resnetaa50d",
            "resnetblur50",
            "resnetblur50d",
            "resnetrs270",
            "resnetrs350",
            "resnetrs420",
            "resnetrs50",
            "resnetv2_50d",
            "resnetv2_50d_evos",
            "resnetv2_50d_frn",
            "resnetv2_50d_gn",
            "resnetv2_50t",
            "resnetv2_50x1_bit",
            "resnetv2_50x3_bit",
            "skresnet50d",
            "test_resnet",
            "tresnet_v2_l",
            "vit_small_resnet50d_s16_224",
            "wide_resnet50_2",
        ),
        resnet.ResNet,
        (resnet.Bottleneck, (3, 4, 6, 3)),
        {"se": False, "groups": 1, "base_width": 64},
        (3, 224, 224),
    ),
    (
        ("davit_giant", "vit_giant_patch16_gap_224", "vit_giant_patch16_lingbot"),
        vision_transformer.VisionTransformer,
        (),
        {"img_size": 224, "patch_size": 16, "embed_dim": 1408, "depth": 40, "num_heads": 16},
        (3, 224, 224),
    ),
    (
        (
            "davit_huge",
            "davit_huge_fl",
            "mvitv2_huge_cls",
            "samvit_huge_patch16",
            "vit_huge_plus_patch16_dinov3",
            "vit_huge_plus_patch16_dinov3_qkvb",
        ),
        vision_transformer.VisionTransformer,
        (),
        {"img_size": 224, "patch_size": 16, "embed_dim": 1280, "depth": 32, "num_heads": 16},
        (3, 224, 224),
    ),
    (
        (
            "deit3_medium_patch16_224",
            "vit_medium_patch16_clip_224",
            "vit_relpos_medium_patch16_224",
            "vit_relpos_medium_patch16_cls_224",
            "vit_relpos_medium_patch16_rpn_224",
            "vit_srelpos_medium_patch16_224",
        ),
        vision_transformer.VisionTransformer,
        (),
        {"img_size": 224, "patch_size": 16, "embed_dim": 512, "depth": 12, "num_heads": 8},
        (3, 224, 224),
    ),
    (
        (
            "deit3_small_patch16_384",
            "maxvit_small_tf_384",
            "vit_pe_core_small_patch16_384",
            "vit_small_patch16_384",
            "vit_small_r26_s32_384",
        ),
        vision_transformer.VisionTransformer,
        (),
        {"img_size": 384, "patch_size": 16, "embed_dim": 384, "depth": 12, "num_heads": 6},
        (3, 384, 384),
    ),
    (
        (
            "deit_small_distilled_patch16_224",
            "flexivit_small",
            "maxvit_rmlp_small_rw_224",
            "maxvit_small_tf_224",
            "mvitv2_small_cls",
            "vit_relpos_small_patch16_rpn_224",
            "vit_small_patch16_18x2_224",
            "vit_small_patch16_36x1_224",
            "vit_small_patch16_dinov3",
            "vit_small_patch16_dinov3_qkvb",
            "vit_small_patch16_lingbot",
            "vit_small_patch16_rope_224",
            "vit_small_patch16_rope_ape_224",
            "vit_small_patch16_rope_mixed_224",
            "vit_small_patch16_rope_mixed_ape_224",
            "vit_small_plus_patch16_dinov3",
            "vit_small_plus_patch16_dinov3_qkvb",
            "vit_small_r26_s32_224",
            "vit_srelpos_small_patch16_224",
            "vit_xsmall_patch16_clip_224",
        ),
        vision_transformer.VisionTransformer,
        (),
        {"img_size": 224, "patch_size": 16, "embed_dim": 384, "depth": 12, "num_heads": 6},
        (3, 224, 224),
    ),
    (
        (
            "deit_tiny_distilled_patch16_224",
            "gcvit_xtiny",
            "gcvit_xxtiny",
            "maxvit_tiny_tf_224",
            "vit_tiny_patch16_dinov3_qkvb",
        ),
        vision_transformer.VisionTransformer,
        (),
        {"img_size": 224, "patch_size": 16, "embed_dim": 192, "depth": 12, "num_heads": 3},
        (3, 224, 224),
    ),
    (
        ("densenet161", "densenet264d", "densenetblur121d"),
        densenet.DenseNet,
        (32, (6, 12, 24, 16)),
        {},
        (3, 224, 224),
    ),
    (
        (
            "dla102x",
            "dla102x2",
            "dla46_c",
            "dla46x_c",
            "dla60_res2net",
            "dla60_res2next",
            "dla60x",
            "dla60x_c",
        ),
        dla.DLA,
        ((1, 1, 1, 2, 2, 1), (16, 32, 64, 128, 256, 512), dla.DlaBasic),
        {},
        (3, 224, 224),
    ),
    (
        (
            "dm_nfnet_f0",
            "dm_nfnet_f1",
            "dm_nfnet_f2",
            "dm_nfnet_f3",
            "dm_nfnet_f4",
            "dm_nfnet_f5",
            "dm_nfnet_f6",
            "eca_nfnet_l0",
            "eca_nfnet_l1",
            "eca_nfnet_l2",
            "eca_nfnet_l3",
            "nf_regnet_b0",
            "nf_regnet_b1",
            "nf_regnet_b2",
            "nf_regnet_b3",
            "nf_regnet_b4",
            "nf_regnet_b5",
            "nfnet_f4",
            "nfnet_f5",
            "nfnet_f6",
            "nfnet_f7",
            "nfnet_l0",
            "test_nfnet",
        ),
        nfnet.NFNet,
        ((128, 256, 512, 1536), (1, 2, 6, 3)),
        {},
        (3, 224, 224),
    ),
    (
        ("dpn48b",),
        dpn.DPN,
        ((10, 10, 10, 10), (16, 32, 32, 64), 16, 16, True, 128),
        {},
        (3, 224, 224),
    ),
    (
        (
            "eca_halonext26ts",
            "gernet_l",
            "gernet_m",
            "gernet_s",
            "halonet26t",
            "halonet50ts",
            "halonet_h1",
            "haloregnetz_b",
            "mobileone_s0",
            "mobileone_s1",
            "mobileone_s2",
            "mobileone_s3",
            "mobileone_s4",
            "regnetz_b16",
            "regnetz_b16_evos",
            "regnetz_c16",
            "regnetz_c16_evos",
            "regnetz_d32",
            "regnetz_d8",
            "regnetz_d8_evos",
            "regnetz_e8",
            "repvgg_a0",
            "repvgg_a1",
            "repvgg_a2",
            "repvgg_b0",
            "repvgg_b1",
            "repvgg_b1g4",
            "repvgg_b2",
            "repvgg_b2g4",
            "repvgg_b3",
            "repvgg_b3g4",
            "repvgg_d2se",
            "sehalonet33ts",
            "test_byobnet",
        ),
        byobnet.ByobNet,
        ((64, 128, 256, 512), (2, 2, 6, 2), 4, (1, 2, 2, 2)),
        {},
        (3, 224, 224),
    ),
    (
        (
            "eca_resnet33ts",
            "ecaresnet50d",
            "ecaresnet50d_pruned",
            "ecaresnet50t",
            "ecaresnetlight",
            "legacy_seresnet50",
            "nf_ecaresnet50",
            "nf_seresnet50",
            "seresnet33ts",
            "seresnet50t",
            "seresnetaa50d",
            "vit_base_resnet50d_224",
        ),
        resnet.ResNet,
        (resnet.Bottleneck, (3, 4, 6, 3)),
        {"se": True, "groups": 1, "base_width": 64},
        (3, 224, 224),
    ),
    (
        (
            "eca_resnext26ts",
            "ecaresnet269d",
            "ecaresnet26t",
            "nf_ecaresnet26",
            "nf_seresnet26",
            "seresnet269d",
            "seresnext26ts",
            "vit_base_resnet26d_224",
        ),
        resnet.ResNet,
        (resnet.Bottleneck, (2, 2, 2, 2)),
        {"se": True, "groups": 1, "base_width": 64},
        (3, 224, 224),
    ),
    (
        ("eca_vovnet39b", "ese_vovnet39b_evos"),
        vovnet.VoVNet,
        (vovnet._CFGS["ese_vovnet39b"][0], True),
        {},
        (3, 224, 224),
    ),
    (
        (
            "ecaresnet101d",
            "ecaresnet101d_pruned",
            "legacy_seresnet101",
            "nf_ecaresnet101",
            "nf_seresnet101",
            "seresnet101",
        ),
        resnet.ResNet,
        (resnet.Bottleneck, (3, 4, 23, 3)),
        {"se": True, "groups": 1, "base_width": 64},
        (3, 224, 224),
    ),
    (
        ("ecaresnet200d", "seresnet200d"),
        resnet.ResNet,
        (resnet.Bottleneck, (3, 24, 36, 3)),
        {"se": True, "groups": 1, "base_width": 64},
        (3, 224, 224),
    ),
    (
        (
            "ecaresnext26t_32x4d",
            "legacy_seresnext26_32x4d",
            "seresnext26d_32x4d",
            "seresnext26t_32x4d",
        ),
        resnet.ResNet,
        (resnet.Bottleneck, (2, 2, 2, 2)),
        {"se": True, "groups": 32, "base_width": 64},
        (3, 224, 224),
    ),
    (
        ("ecaresnext50t_32x4d", "legacy_seresnext50_32x4d"),
        resnet.ResNet,
        (resnet.Bottleneck, (3, 4, 6, 3)),
        {"se": True, "groups": 32, "base_width": 64},
        (3, 224, 224),
    ),
    (
        (
            "efficientnet_b0",
            "efficientnet_b0_g16_evos",
            "efficientnet_b0_g8_gn",
            "efficientnet_b0_gn",
            "efficientnet_blur_b0",
            "efficientnet_cc_b0_4e",
            "efficientnet_cc_b0_8e",
            "efficientnet_el",
            "efficientnet_el_pruned",
            "efficientnet_em",
            "efficientnet_es",
            "efficientnet_es_pruned",
            "efficientnet_l2",
            "efficientnet_lite0",
            "efficientnet_lite1",
            "efficientnet_lite2",
            "efficientnet_lite3",
            "efficientnet_lite4",
            "efficientnetv2_l",
            "efficientnetv2_m",
            "efficientnetv2_rw_m",
            "efficientnetv2_rw_s",
            "efficientnetv2_rw_t",
            "efficientnetv2_s",
            "efficientnetv2_xl",
            "fbnetv3_b",
            "fbnetv3_d",
            "fbnetv3_g",
            "gc_efficientnetv2_rw_t",
            "lcnet_035",
            "mixnet_l",
            "mixnet_m",
            "mixnet_s",
            "mixnet_xl",
            "mixnet_xxl",
            "mobilenet_edgetpu_v2_l",
            "mobilenet_edgetpu_v2_m",
            "mobilenet_edgetpu_v2_s",
            "mobilenet_edgetpu_v2_xs",
            "mobilenetv1_125",
            "mobilenetv2_035",
            "mobilenetv2_110d",
            "mobilenetv2_120d",
            "mobilenetv3_rw",
            "mobilenetv4_conv_aa_large",
            "mobilenetv4_conv_aa_medium",
            "mobilenetv4_conv_blur_medium",
            "mobilenetv4_conv_large",
            "mobilenetv4_conv_medium",
            "mobilenetv4_hybrid_large",
            "mobilenetv4_hybrid_medium",
            "test_efficientnet",
            "test_efficientnet_evos",
            "test_efficientnet_gn",
            "test_efficientnet_ln",
            "tf_efficientnet_b0",
            "tf_efficientnet_cc_b0_4e",
            "tf_efficientnet_cc_b0_8e",
            "tf_efficientnet_el",
            "tf_efficientnet_em",
            "tf_efficientnet_es",
            "tf_efficientnet_l2",
            "tf_efficientnet_lite0",
            "tf_efficientnet_lite1",
            "tf_efficientnet_lite2",
            "tf_efficientnet_lite3",
            "tf_efficientnet_lite4",
            "tf_efficientnetv2_b0",
            "tf_efficientnetv2_l",
            "tf_efficientnetv2_m",
            "tf_efficientnetv2_s",
            "tf_efficientnetv2_xl",
            "tf_mixnet_l",
            "tf_mixnet_m",
            "tf_mixnet_s",
        ),
        efficientnet.EfficientNet,
        (),
        {"channel_multiplier": 1.0, "depth_multiplier": 1.0},
        (3, 224, 224),
    ),
    (
        (
            "efficientnet_b1",
            "efficientnet_b1_pruned",
            "efficientnet_cc_b1_8e",
            "lcnet_075",
            "mnasnet_075",
            "mobilenetv2_075",
            "mobilenetv3_large_075",
            "mobilenetv3_small_075",
            "mobilenetv4_hybrid_large_075",
            "mobilenetv4_hybrid_medium_075",
            "semnasnet_075",
            "tf_efficientnet_b1",
            "tf_efficientnet_cc_b1_8e",
            "tf_efficientnetv2_b1",
            "tf_mobilenetv3_large_075",
            "tf_mobilenetv3_small_075",
        ),
        efficientnet.EfficientNet,
        (),
        {"channel_multiplier": 1.0, "depth_multiplier": 1.1},
        (3, 224, 224),
    ),
    (
        (
            "efficientnet_b2",
            "efficientnet_b2_pruned",
            "fbnetc_100",
            "lcnet_100",
            "mobilenet_edgetpu_100",
            "mobilenetv1_100",
            "mobilenetv1_100h",
            "semnasnet_100",
            "tf_efficientnet_b2",
            "tf_efficientnetv2_b2",
            "tf_mobilenetv3_large_100",
            "tf_mobilenetv3_large_minimal_100",
            "tf_mobilenetv3_small_100",
            "tf_mobilenetv3_small_minimal_100",
        ),
        efficientnet.EfficientNet,
        (),
        {"channel_multiplier": 1.1, "depth_multiplier": 1.2},
        (3, 224, 224),
    ),
    (
        (
            "efficientnet_b3",
            "efficientnet_b3_g8_gn",
            "efficientnet_b3_gn",
            "efficientnet_b3_pruned",
            "efficientnet_x_b3",
            "tf_efficientnet_b3",
            "tf_efficientnetv2_b3",
        ),
        efficientnet.EfficientNet,
        (),
        {"channel_multiplier": 1.2, "depth_multiplier": 1.4},
        (3, 224, 224),
    ),
    (
        ("efficientnet_b4", "semnasnet_140", "tf_efficientnet_b4"),
        efficientnet.EfficientNet,
        (),
        {"channel_multiplier": 1.4, "depth_multiplier": 1.8},
        (3, 224, 224),
    ),
    (
        (
            "efficientnet_b5",
            "efficientnet_h_b5",
            "efficientnet_x_b5",
            "lcnet_150",
            "mobilenetv3_large_150d",
            "tf_efficientnet_b5",
        ),
        efficientnet.EfficientNet,
        (),
        {"channel_multiplier": 1.6, "depth_multiplier": 2.2},
        (3, 224, 224),
    ),
    (
        ("efficientnet_b6", "tf_efficientnet_b6"),
        efficientnet.EfficientNet,
        (),
        {"channel_multiplier": 1.8, "depth_multiplier": 2.6},
        (3, 224, 224),
    ),
    (
        ("efficientnet_b7", "tf_efficientnet_b7"),
        efficientnet.EfficientNet,
        (),
        {"channel_multiplier": 2.0, "depth_multiplier": 3.1},
        (3, 224, 224),
    ),
    (
        ("efficientnet_b8", "tf_efficientnet_b8"),
        efficientnet.EfficientNet,
        (),
        {"channel_multiplier": 2.2, "depth_multiplier": 3.6},
        (3, 224, 224),
    ),
    (
        ("ese_vovnet19b_dw", "ese_vovnet19b_slim", "ese_vovnet19b_slim_dw"),
        vovnet.VoVNet,
        (vovnet._CFGS["ese_vovnet19b"][0], True),
        {},
        (3, 224, 224),
    ),
    (
        ("ese_vovnet57b", "ese_vovnet99b"),
        vovnet.VoVNet,
        (vovnet._CFGS["vovnet57a"][0], True),
        {},
        (3, 224, 224),
    ),
    (
        (
            "eva02_base_patch14_224",
            "eva02_base_patch16_clip_224",
            "eva02_enormous_patch14_clip_224",
        ),
        eva.Eva,
        (),
        {"img_size": 224, "embed_dim": 768, "depth": 12, "num_heads": 12},
        (3, 224, 224),
    ),
    (
        ("eva02_base_patch14_448",),
        eva.Eva,
        (),
        {"img_size": 448, "embed_dim": 768, "depth": 12, "num_heads": 12},
        (3, 448, 448),
    ),
    (
        ("eva02_large_patch14_224", "eva02_large_patch14_clip_224"),
        eva.Eva,
        (),
        {"img_size": 224, "embed_dim": 1024, "depth": 24, "num_heads": 16},
        (3, 224, 224),
    ),
    (
        ("eva02_large_patch14_448",),
        eva.Eva,
        (),
        {"img_size": 448, "embed_dim": 1024, "depth": 24, "num_heads": 16},
        (3, 448, 448),
    ),
    (
        ("eva02_large_patch14_clip_336",),
        eva.Eva,
        (),
        {"img_size": 336, "embed_dim": 1024, "depth": 24, "num_heads": 16},
        (3, 336, 336),
    ),
    (
        ("eva02_small_patch14_224",),
        eva.Eva,
        (),
        {"img_size": 224, "embed_dim": 384, "depth": 12, "num_heads": 6},
        (3, 224, 224),
    ),
    (
        ("eva02_small_patch14_336",),
        eva.Eva,
        (),
        {"img_size": 336, "embed_dim": 384, "depth": 12, "num_heads": 6},
        (3, 336, 336),
    ),
    (
        ("eva02_tiny_patch14_224",),
        eva.Eva,
        (),
        {"img_size": 224, "embed_dim": 192, "depth": 12, "num_heads": 3},
        (3, 224, 224),
    ),
    (
        ("eva02_tiny_patch14_336",),
        eva.Eva,
        (),
        {"img_size": 336, "embed_dim": 192, "depth": 12, "num_heads": 3},
        (3, 336, 336),
    ),
    (
        ("eva_giant_patch14_224", "eva_giant_patch14_clip_224"),
        eva.Eva,
        (),
        {"img_size": 224, "embed_dim": 1408, "depth": 40, "num_heads": 16},
        (3, 224, 224),
    ),
    (
        ("eva_giant_patch14_336",),
        eva.Eva,
        (),
        {"img_size": 336, "embed_dim": 1408, "depth": 40, "num_heads": 16},
        (3, 336, 336),
    ),
    (
        ("eva_giant_patch14_560",),
        eva.Eva,
        (),
        {"img_size": 560, "embed_dim": 1408, "depth": 40, "num_heads": 16},
        (3, 560, 560),
    ),
    (
        ("ghostnetv2_160", "ghostnetv3_160"),
        resnet.ResNet,
        (resnet.Bottleneck, (3, 4, 6, 3)),
        {},
        (3, 160, 160),
    ),
    (
        (
            "gmixer_12_224",
            "gmixer_24_224",
            "gmlp_b16_224",
            "gmlp_s16_224",
            "gmlp_ti16_224",
            "mixer_l32_224",
            "mixer_s16_224",
            "mixer_s32_224",
            "resmlp_big_24_224",
        ),
        mlp_mixer.MlpMixer,
        (),
        {"patch_size": 16, "embed_dim": 512, "num_blocks": 8},
        (3, 224, 224),
    ),
    (
        ("hiera_base_abswin_256", "hiera_small_abswin_256", "swinv2_cr_small_ns_256"),
        resnet.ResNet,
        (resnet.Bottleneck, (3, 4, 6, 3)),
        {},
        (3, 256, 256),
    ),
    (
        (
            "hrnet_w18_small_v2",
            "hrnet_w18_ssld",
            "hrnet_w30",
            "hrnet_w40",
            "hrnet_w44",
            "hrnet_w48_ssld",
            "hrnet_w64",
        ),
        hrnet.HRNet,
        ((18, 36, 72, 144), 1, 1, 4, 3, False),
        {},
        (3, 224, 224),
    ),
    (
        ("lambda_resnet26rpt_256",),
        resnet.ResNet,
        (resnet.Bottleneck, (2, 2, 2, 2)),
        {"se": False, "groups": 1, "base_width": 64},
        (3, 256, 256),
    ),
    (
        (
            "lcnet_050",
            "mnasnet_small",
            "mobilenetv3_small_050",
            "mobilenetv4_conv_small",
            "mobilenetv4_conv_small_035",
            "mobilenetv4_conv_small_050",
            "semnasnet_050",
            "tinynet_a",
            "tinynet_b",
            "tinynet_c",
            "tinynet_d",
            "tinynet_e",
        ),
        efficientnet.EfficientNet,
        (),
        {"channel_multiplier": 0.5, "depth_multiplier": 0.8},
        (3, 224, 224),
    ),
    (("legacy_senet154",), senet.SENet154, (), {}, (3, 224, 224)),
    (
        ("legacy_seresnet152", "seresnet152", "seresnet152d"),
        resnet.ResNet,
        (resnet.Bottleneck, (3, 8, 36, 3)),
        {"se": True, "groups": 1, "base_width": 64},
        (3, 224, 224),
    ),
    (
        ("legacy_seresnet18", "seresnet18"),
        resnet.ResNet,
        (resnet.BasicBlock, (2, 2, 2, 2)),
        {"se": True, "groups": 1, "base_width": 64},
        (3, 224, 224),
    ),
    (
        ("legacy_seresnet34", "seresnet34"),
        resnet.ResNet,
        (resnet.BasicBlock, (3, 4, 6, 3)),
        {"se": True, "groups": 1, "base_width": 64},
        (3, 224, 224),
    ),
    (
        ("legacy_seresnext101_32x4d", "seresnext101_32x4d"),
        resnet.ResNet,
        (resnet.Bottleneck, (3, 4, 23, 3)),
        {"se": True, "groups": 32, "base_width": 64},
        (3, 224, 224),
    ),
    (
        (
            "levit_256d",
            "levit_conv_256",
            "levit_conv_256d",
            "maxvit_nano_rw_256",
            "maxvit_pico_rw_256",
            "maxvit_rmlp_nano_rw_256",
            "maxvit_rmlp_pico_rw_256",
            "maxxvit_rmlp_nano_rw_256",
            "maxxvitv2_nano_rw_256",
            "vit_base_patch16_reg4_gap_256",
            "vit_base_patch16_rope_reg1_gap_256",
            "vit_base_patch16_siglip_256",
            "vit_base_patch16_siglip_gap_256",
            "vit_betwixt_patch16_gap_256",
            "vit_betwixt_patch16_reg1_gap_256",
            "vit_betwixt_patch16_reg4_gap_256",
            "vit_betwixt_patch16_rope_reg4_gap_256",
            "vit_dlittle_patch16_reg1_gap_256",
            "vit_dpwee_patch16_reg1_gap_256",
            "vit_dwee_patch16_reg1_gap_256",
            "vit_little_patch16_reg1_gap_256",
            "vit_little_patch16_reg4_gap_256",
            "vit_pwee_patch16_reg1_gap_256",
            "vit_so150m2_patch16_reg1_gap_256",
            "vit_so150m_patch16_reg4_gap_256",
            "vit_so150m_patch16_reg4_map_256",
            "vit_so400m_patch16_siglip_256",
            "vit_so400m_patch16_siglip_gap_256",
            "vit_wee_patch16_reg1_gap_256",
        ),
        vision_transformer.VisionTransformer,
        (),
        {"img_size": 256, "patch_size": 16, "embed_dim": 768, "depth": 12, "num_heads": 12},
        (3, 256, 256),
    ),
    (
        (
            "levit_512",
            "levit_512_s8",
            "levit_512d",
            "levit_conv_512",
            "levit_conv_512_s8",
            "levit_conv_512d",
            "maxvit_base_tf_512",
            "vit_base_patch16_siglip_512",
            "vit_base_patch16_siglip_gap_512",
            "vit_pe_spatial_base_patch16_512",
            "vit_so400m_patch16_siglip_512",
            "vit_so400m_patch16_siglip_gap_512",
        ),
        vision_transformer.VisionTransformer,
        (),
        {"img_size": 512, "patch_size": 16, "embed_dim": 768, "depth": 12, "num_heads": 12},
        (3, 512, 512),
    ),
    (
        ("levit_conv_192",),
        vision_transformer.VisionTransformer,
        (),
        {"img_size": 192, "patch_size": 16, "embed_dim": 768, "depth": 12, "num_heads": 12},
        (3, 192, 192),
    ),
    (
        ("maxvit_rmlp_small_rw_256", "maxxvit_rmlp_small_rw_256"),
        vision_transformer.VisionTransformer,
        (),
        {"img_size": 256, "patch_size": 16, "embed_dim": 384, "depth": 12, "num_heads": 6},
        (3, 256, 256),
    ),
    (
        (
            "maxvit_rmlp_tiny_rw_256",
            "maxvit_tiny_pm_256",
            "maxvit_tiny_rw_256",
            "maxxvit_rmlp_tiny_rw_256",
        ),
        vision_transformer.VisionTransformer,
        (),
        {"img_size": 256, "patch_size": 16, "embed_dim": 192, "depth": 12, "num_heads": 3},
        (3, 256, 256),
    ),
    (
        ("maxvit_small_tf_512", "vit_pe_spatial_small_patch16_512"),
        vision_transformer.VisionTransformer,
        (),
        {"img_size": 512, "patch_size": 16, "embed_dim": 384, "depth": 12, "num_heads": 6},
        (3, 512, 512),
    ),
    (
        (
            "maxvit_tiny_tf_384",
            "tiny_vit_21m_384",
            "vit_pe_core_tiny_patch16_384",
            "vit_tiny_patch16_384",
        ),
        vision_transformer.VisionTransformer,
        (),
        {"img_size": 384, "patch_size": 16, "embed_dim": 192, "depth": 12, "num_heads": 3},
        (3, 384, 384),
    ),
    (
        ("maxvit_tiny_tf_512", "tiny_vit_21m_512", "vit_pe_spatial_tiny_patch16_512"),
        vision_transformer.VisionTransformer,
        (),
        {"img_size": 512, "patch_size": 16, "embed_dim": 192, "depth": 12, "num_heads": 3},
        (3, 512, 512),
    ),
    (
        (
            "nf_resnet101",
            "resnet101_clip",
            "resnet101_clip_gap",
            "resnet101c",
            "resnet101d",
            "resnet101s",
            "resnetaa101d",
            "resnetblur101d",
            "resnetrs101",
            "resnetv2_101d",
            "resnetv2_101x1_bit",
            "resnetv2_101x3_bit",
            "wide_resnet101_2",
        ),
        resnet.ResNet,
        (resnet.Bottleneck, (3, 4, 23, 3)),
        {"se": False, "groups": 1, "base_width": 64},
        (3, 224, 224),
    ),
    (
        (
            "regnetv_040",
            "regnetv_064",
            "regnetx_004_tv",
            "regnetx_006",
            "regnetx_040",
            "regnetx_064",
            "regnetx_080",
            "regnetx_120",
            "regnety_002",
            "regnety_006",
            "regnety_040",
            "regnety_040_sgn",
            "regnety_064",
            "regnety_080",
            "regnety_080_tv",
            "regnety_120",
            "regnety_1280",
            "regnety_640",
            "regnetz_005",
            "regnetz_040",
            "regnetz_040_h",
        ),
        regnet.RegNet,
        (*regnet.gen_cfg(10, 32, 16.0, 2.0, 8),),
        {},
        (3, 224, 224),
    ),
    (
        ("regnetx_160", "regnety_160"),
        regnet.RegNet,
        (*regnet.gen_cfg(10, 32, 16.0, 2.0, 8),),
        {},
        (3, 160, 160),
    ),
    (
        ("regnetx_320", "regnety_320"),
        regnet.RegNet,
        (*regnet.gen_cfg(10, 32, 16.0, 2.0, 8),),
        {},
        (3, 320, 320),
    ),
    (
        ("regnety_008_tv",),
        regnet.RegNet,
        (*regnet.gen_cfg(16, 48, 27.89, 2.09, 8),),
        {"se_ratio": 0.25},
        (3, 224, 224),
    ),
    (("regnety_2560",), regnet.RegNet, (*regnet.gen_cfg(10, 32, 16.0, 2.0, 8),), {}, (3, 256, 256)),
    (
        (
            "resnet152c",
            "resnet152d",
            "resnet152s",
            "resnetrs152",
            "resnetv2_152d",
            "resnetv2_152x2_bit",
            "resnetv2_152x4_bit",
        ),
        resnet.ResNet,
        (resnet.Bottleneck, (3, 8, 36, 3)),
        {"se": False, "groups": 1, "base_width": 64},
        (3, 224, 224),
    ),
    (
        ("resnet18d", "resnetblur18", "resnetv2_18", "resnetv2_18d", "skresnet18"),
        resnet.ResNet,
        (resnet.BasicBlock, (2, 2, 2, 2)),
        {"se": False, "groups": 1, "base_width": 64},
        (3, 224, 224),
    ),
    (
        ("resnet200", "resnet200d", "resnetrs200"),
        resnet.ResNet,
        (resnet.Bottleneck, (3, 24, 36, 3)),
        {"se": False, "groups": 1, "base_width": 64},
        (3, 224, 224),
    ),
    (
        ("resnet34d", "resnetaa34d", "resnetv2_34", "resnetv2_34d", "skresnet34"),
        resnet.ResNet,
        (resnet.BasicBlock, (3, 4, 6, 3)),
        {"se": False, "groups": 1, "base_width": 64},
        (3, 224, 224),
    ),
    (
        ("resnext101_32x16d",),
        resnet.ResNet,
        (resnet.Bottleneck, (3, 4, 23, 3)),
        {"se": False, "groups": 32, "base_width": 16},
        (3, 224, 224),
    ),
    (
        ("resnext101_32x32d",),
        resnet.ResNet,
        (resnet.Bottleneck, (3, 4, 23, 3)),
        {"se": False, "groups": 32, "base_width": 32},
        (3, 224, 224),
    ),
    (
        ("resnext101_32x4d",),
        resnet.ResNet,
        (resnet.Bottleneck, (3, 4, 23, 3)),
        {"se": False, "groups": 32, "base_width": 64},
        (3, 224, 224),
    ),
    (
        ("resnext101_64x4d",),
        resnet.ResNet,
        (resnet.Bottleneck, (3, 4, 23, 3)),
        {"se": False, "groups": 64, "base_width": 64},
        (3, 224, 224),
    ),
    (
        ("resnext50d_32x4d", "skresnext50_32x4d"),
        resnet.ResNet,
        (resnet.Bottleneck, (3, 4, 6, 3)),
        {"se": False, "groups": 32, "base_width": 64},
        (3, 224, 224),
    ),
    (
        ("seresnext101_32x8d", "seresnext101d_32x8d", "seresnextaa101d_32x8d"),
        resnet.ResNet,
        (resnet.Bottleneck, (3, 4, 23, 3)),
        {"se": True, "groups": 32, "base_width": 8},
        (3, 224, 224),
    ),
    (
        ("seresnext101_64x4d",),
        resnet.ResNet,
        (resnet.Bottleneck, (3, 4, 23, 3)),
        {"se": True, "groups": 64, "base_width": 64},
        (3, 224, 224),
    ),
    (
        ("seresnextaa201d_32x8d",),
        resnet.ResNet,
        (resnet.Bottleneck, (3, 4, 6, 3)),
        {"se": True, "groups": 32, "base_width": 8},
        (3, 224, 224),
    ),
    (
        ("swin_base_patch4_window12_384",),
        swin_transformer.SwinTransformer,
        (),
        {
            "img_size": 384,
            "embed_dim": 128,
            "depths": (2, 2, 18, 2),
            "num_heads": (4, 8, 16, 32),
            "window_size": 12,
        },
        (3, 384, 384),
    ),
    (
        ("swin_large_patch4_window12_384",),
        swin_transformer.SwinTransformer,
        (),
        {
            "img_size": 384,
            "embed_dim": 192,
            "depths": (2, 2, 18, 2),
            "num_heads": (6, 12, 24, 48),
            "window_size": 12,
        },
        (3, 384, 384),
    ),
    (
        ("swin_large_patch4_window7_224",),
        swin_transformer.SwinTransformer,
        (),
        {"img_size": 224, "embed_dim": 192, "depths": (2, 2, 18, 2), "num_heads": (6, 12, 24, 48)},
        (3, 224, 224),
    ),
    (
        ("swin_s3_base_224",),
        swin_transformer.SwinTransformer,
        (),
        {"img_size": 224, "embed_dim": 128, "depths": (2, 2, 18, 2), "num_heads": (4, 8, 16, 32)},
        (3, 224, 224),
    ),
    (
        ("swin_s3_small_224",),
        swin_transformer.SwinTransformer,
        (),
        {"img_size": 224, "embed_dim": 96, "depths": (2, 2, 18, 2), "num_heads": (3, 6, 12, 24)},
        (3, 224, 224),
    ),
    (
        ("swin_s3_tiny_224",),
        swin_transformer.SwinTransformer,
        (),
        {"img_size": 224, "embed_dim": 96, "depths": (2, 2, 6, 2), "num_heads": (3, 6, 12, 24)},
        (3, 224, 224),
    ),
    (
        (
            "swinv2_base_window12_192",
            "swinv2_base_window12to16_192to256",
            "swinv2_base_window12to24_192to384",
        ),
        swin_transformer.SwinTransformer,
        (),
        {
            "img_size": 192,
            "embed_dim": 128,
            "depths": (2, 2, 18, 2),
            "num_heads": (4, 8, 16, 32),
            "window_size": 12,
            "block_cls": swin_transformer.SwinV2Block,
        },
        (3, 192, 192),
    ),
    (
        ("swinv2_base_window16_256",),
        swin_transformer.SwinTransformer,
        (),
        {
            "img_size": 256,
            "embed_dim": 128,
            "depths": (2, 2, 18, 2),
            "num_heads": (4, 8, 16, 32),
            "window_size": 16,
            "block_cls": swin_transformer.SwinV2Block,
        },
        (3, 256, 256),
    ),
    (
        ("swinv2_base_window8_256",),
        swin_transformer.SwinTransformer,
        (),
        {
            "img_size": 256,
            "embed_dim": 128,
            "depths": (2, 2, 18, 2),
            "num_heads": (4, 8, 16, 32),
            "window_size": 8,
            "block_cls": swin_transformer.SwinV2Block,
        },
        (3, 256, 256),
    ),
    (
        (
            "swinv2_large_window12_192",
            "swinv2_large_window12to16_192to256",
            "swinv2_large_window12to24_192to384",
        ),
        swin_transformer.SwinTransformer,
        (),
        {
            "img_size": 192,
            "embed_dim": 192,
            "depths": (2, 2, 18, 2),
            "num_heads": (6, 12, 24, 48),
            "window_size": 12,
            "block_cls": swin_transformer.SwinV2Block,
        },
        (3, 192, 192),
    ),
    (
        ("swinv2_small_window16_256",),
        swin_transformer.SwinTransformer,
        (),
        {
            "img_size": 256,
            "embed_dim": 96,
            "depths": (2, 2, 18, 2),
            "num_heads": (3, 6, 12, 24),
            "window_size": 16,
            "block_cls": swin_transformer.SwinV2Block,
        },
        (3, 256, 256),
    ),
    (
        ("swinv2_tiny_window16_256",),
        swin_transformer.SwinTransformer,
        (),
        {
            "img_size": 256,
            "embed_dim": 96,
            "depths": (2, 2, 6, 2),
            "num_heads": (3, 6, 12, 24),
            "window_size": 16,
            "block_cls": swin_transformer.SwinV2Block,
        },
        (3, 256, 256),
    ),
    (
        (
            "vit_base_patch32_224",
            "vit_base_patch32_clip_224",
            "vit_base_patch32_clip_quickgelu_224",
            "vit_betwixt_patch32_clip_224",
        ),
        vision_transformer.VisionTransformer,
        (),
        {"img_size": 224, "patch_size": 32, "embed_dim": 768, "depth": 12, "num_heads": 12},
        (3, 224, 224),
    ),
    (
        ("vit_base_patch32_384", "vit_base_patch32_clip_384"),
        vision_transformer.VisionTransformer,
        (),
        {"img_size": 384, "patch_size": 32, "embed_dim": 768, "depth": 12, "num_heads": 12},
        (3, 384, 384),
    ),
    (
        (
            "vit_base_patch32_clip_256",
            "vit_base_patch32_plus_256",
            "vit_base_patch32_siglip_256",
            "vit_base_patch32_siglip_gap_256",
            "vit_relpos_base_patch32_plus_rpn_256",
        ),
        vision_transformer.VisionTransformer,
        (),
        {"img_size": 256, "patch_size": 32, "embed_dim": 768, "depth": 12, "num_heads": 12},
        (3, 256, 256),
    ),
    (
        ("vit_base_patch32_clip_448",),
        vision_transformer.VisionTransformer,
        (),
        {"img_size": 448, "patch_size": 32, "embed_dim": 768, "depth": 12, "num_heads": 12},
        (3, 448, 448),
    ),
    (
        ("vit_base_patch8_224",),
        vision_transformer.VisionTransformer,
        (),
        {"img_size": 224, "patch_size": 8, "embed_dim": 768, "depth": 12, "num_heads": 12},
        (3, 224, 224),
    ),
    (
        ("vit_giantopt_patch16_siglip_256", "vit_giantopt_patch16_siglip_gap_256"),
        vision_transformer.VisionTransformer,
        (),
        {"img_size": 256, "patch_size": 16, "embed_dim": 1408, "depth": 40, "num_heads": 16},
        (3, 256, 256),
    ),
    (
        ("vit_giantopt_patch16_siglip_384", "vit_giantopt_patch16_siglip_gap_384"),
        vision_transformer.VisionTransformer,
        (),
        {"img_size": 384, "patch_size": 16, "embed_dim": 1408, "depth": 40, "num_heads": 16},
        (3, 384, 384),
    ),
    (
        ("vit_huge_patch16_gap_448",),
        vision_transformer.VisionTransformer,
        (),
        {"img_size": 448, "patch_size": 16, "embed_dim": 1280, "depth": 32, "num_heads": 16},
        (3, 448, 448),
    ),
    (
        (
            "vit_large_patch16_siglip_256",
            "vit_large_patch16_siglip_gap_256",
            "vitamin_large2_256",
            "vitamin_large_256",
            "vitamin_xlarge_256",
        ),
        vision_transformer.VisionTransformer,
        (),
        {"img_size": 256, "patch_size": 16, "embed_dim": 1024, "depth": 24, "num_heads": 16},
        (3, 256, 256),
    ),
    (
        ("vit_large_patch32_224",),
        vision_transformer.VisionTransformer,
        (),
        {"img_size": 224, "patch_size": 32, "embed_dim": 1024, "depth": 24, "num_heads": 16},
        (3, 224, 224),
    ),
    (
        ("vit_large_patch32_384",),
        vision_transformer.VisionTransformer,
        (),
        {"img_size": 384, "patch_size": 32, "embed_dim": 1024, "depth": 24, "num_heads": 16},
        (3, 384, 384),
    ),
    (
        ("vit_medium_patch16_gap_240",),
        vision_transformer.VisionTransformer,
        (),
        {"img_size": 240, "patch_size": 16, "embed_dim": 512, "depth": 12, "num_heads": 8},
        (3, 240, 240),
    ),
    (
        (
            "vit_medium_patch16_gap_256",
            "vit_medium_patch16_reg1_gap_256",
            "vit_medium_patch16_reg4_gap_256",
            "vit_medium_patch16_rope_reg1_gap_256",
            "vit_mediumd_patch16_reg4_gap_256",
            "vit_mediumd_patch16_rope_reg1_gap_256",
        ),
        vision_transformer.VisionTransformer,
        (),
        {"img_size": 256, "patch_size": 16, "embed_dim": 512, "depth": 12, "num_heads": 8},
        (3, 256, 256),
    ),
    (
        ("vit_medium_patch16_gap_384", "vit_mediumd_patch16_reg4_gap_384"),
        vision_transformer.VisionTransformer,
        (),
        {"img_size": 384, "patch_size": 16, "embed_dim": 512, "depth": 12, "num_heads": 8},
        (3, 384, 384),
    ),
    (
        ("vit_medium_patch32_clip_224",),
        vision_transformer.VisionTransformer,
        (),
        {"img_size": 224, "patch_size": 32, "embed_dim": 512, "depth": 12, "num_heads": 8},
        (3, 224, 224),
    ),
    (
        ("vit_small_patch14_dinov2", "vit_small_patch14_reg4_dinov2"),
        vision_transformer.VisionTransformer,
        (),
        {"img_size": 224, "patch_size": 14, "embed_dim": 384, "depth": 12, "num_heads": 6},
        (3, 224, 224),
    ),
    (
        ("vit_small_patch32_224",),
        vision_transformer.VisionTransformer,
        (),
        {"img_size": 224, "patch_size": 32, "embed_dim": 384, "depth": 12, "num_heads": 6},
        (3, 224, 224),
    ),
    (
        ("vit_small_patch32_384",),
        vision_transformer.VisionTransformer,
        (),
        {"img_size": 384, "patch_size": 32, "embed_dim": 384, "depth": 12, "num_heads": 6},
        (3, 384, 384),
    ),
    (
        ("vit_small_patch8_224",),
        vision_transformer.VisionTransformer,
        (),
        {"img_size": 224, "patch_size": 8, "embed_dim": 384, "depth": 12, "num_heads": 6},
        (3, 224, 224),
    ),
    (
        ("vit_so150m2_patch16_reg1_gap_448",),
        vision_transformer.VisionTransformer,
        (),
        {"img_size": 448, "patch_size": 16, "embed_dim": 768, "depth": 12, "num_heads": 12},
        (3, 448, 448),
    ),
    (
        ("vit_so400m_patch14_siglip_384", "vit_so400m_patch14_siglip_gap_384"),
        vision_transformer.VisionTransformer,
        (),
        {"img_size": 384, "patch_size": 14, "embed_dim": 768, "depth": 12, "num_heads": 12},
        (3, 384, 384),
    ),
    (
        ("vit_tiny_r_s16_p8_224",),
        vision_transformer.VisionTransformer,
        (),
        {"img_size": 224, "patch_size": 8, "embed_dim": 192, "depth": 12, "num_heads": 3},
        (3, 224, 224),
    ),
    (
        ("vit_tiny_r_s16_p8_384",),
        vision_transformer.VisionTransformer,
        (),
        {"img_size": 384, "patch_size": 8, "embed_dim": 192, "depth": 12, "num_heads": 3},
        (3, 384, 384),
    ),
    (
        ("vitamin_large2_336", "vitamin_large_336", "vitamin_xlarge_336"),
        vision_transformer.VisionTransformer,
        (),
        {"img_size": 336, "patch_size": 16, "embed_dim": 1024, "depth": 24, "num_heads": 16},
        (3, 336, 336),
    ),
    (("volo_d5_512",), resnet.ResNet, (resnet.Bottleneck, (3, 4, 6, 3)), {}, (3, 512, 512)),
    (
        (
            "xcit_large_24_p16_224",
            "xcit_large_24_p8_224",
            "xcit_medium_24_p8_224",
            "xcit_nano_12_p16_224",
            "xcit_nano_12_p8_224",
            "xcit_small_12_p8_224",
            "xcit_small_24_p16_224",
            "xcit_small_24_p8_224",
            "xcit_tiny_12_p8_224",
            "xcit_tiny_24_p16_224",
            "xcit_tiny_24_p8_224",
        ),
        xcit.XCiT,
        (),
        {"embed_dim": 384, "depth": 12, "num_heads": 8},
        (3, 224, 224),
    ),
    (
        (
            "xcit_large_24_p16_384",
            "xcit_large_24_p8_384",
            "xcit_medium_24_p16_384",
            "xcit_medium_24_p8_384",
            "xcit_nano_12_p16_384",
            "xcit_nano_12_p8_384",
            "xcit_small_12_p16_384",
            "xcit_small_12_p8_384",
            "xcit_small_24_p16_384",
            "xcit_small_24_p8_384",
            "xcit_tiny_12_p16_384",
            "xcit_tiny_12_p8_384",
            "xcit_tiny_24_p16_384",
            "xcit_tiny_24_p8_384",
        ),
        xcit.XCiT,
        (),
        {"embed_dim": 384, "depth": 12, "num_heads": 8},
        (3, 384, 384),
    ),
]

for _names, _ctor, _args, _fixed, _input_size in _SPECS:
    for _name in _names:
        globals()[_name] = (
            model_entrypoint(_name)
            if is_model(_name)
            else register_model(_make(_name, _ctor, _args, _fixed, _input_size))
        )
